...
export const fetchSkills = async () => {
  try {
    ...
    const skills = ['Plumbing', 'Electrical', 'Gardening', 'Grocery Shopping', 'Babysitting'];
    return skills;
  } catch (error) {
...
export const fetchUsersBySkill = async (skill: string) => {
  try {
    ...
    const sampleUsers: User[] = [
      { username: 'jdoe', name: 'John Doe', skills: ['Plumbing'], },
      { username: 'jsmith', name: 'Jane Smith', skills: ['Gardening'] },
...
export const submitRequest = async (requestData: { requesterUsername: string, targetUsername: string, title: string, description: string }) => {
  try {
